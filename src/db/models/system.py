...
class SystemFunction(Entity):
    ...
    id: Mapped[int] = mapped_column(primary_key=True)
    group_id: Mapped[int] = mapped_column(ForeignKey(column="sys_func_group.id"))
    description: Mapped[str]
    path: Mapped[str]
    type: Mapped[FunctionType] = mapped_column(
        Enum(FunctionType, native_enum=False, validate_strings=True)
    )
    # relationship
